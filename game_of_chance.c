...
int main() {
  ...
  while(choice != 7) {
    ...
    printf("3 - Play the Find an Ace game\n");
    ...
    else if (choice < 4) {	// Otherwise, choice was one of the games
...
int get_player_data() {
  ...
  if(fd == -1) // Can't open the file, maybe it doesn't exist
    return -1;
  read_bytes = read(fd, &entry, sizeof(struct user));	// Read this first chunk
  ...
  if(read_bytes < sizeof(struct user)) 	// This means the end of file was reached
    return -1;
  else
...
void update_player_data() {
  ...
  read(fd, &read_uid, 4);	// Read the uid from the last struct - line 147
...
void show_highscore() {
  ...
  while(read(fd, &entry, sizeof(struct user)) > 0) {	// Loop until end of file. [line 169]
...
void input_name() {
  ...
  while(input_char != '\n') {	// Loop until new line
    *name_ptr = input_char;	// Put the input char into the name field.
...
// for the second wager in the Find an Ace game. The function
...
int take_wager(int available_credits, int previous_wager) {
  ...
  if(wager < 1) {	// Make sure the wager is greater than 0.
    printf("You must wager a positive number.\n");
    return -1;
  }
  total_wager = previous_wager + wager;
  if(total_wager > available_credits) {	 // confirm available credits
    printf("Your total wager of %d is more than you have available.\n", total_wager);
...
// played again. It also writes the new credit totals to the file
...
void play_the_game() {
  ...
  while(play_again) {
	...
	player.highscore = player.credits;	// update the highscore.
      ...
      update_player_data();			// Write the new credit total to a new file.
	...
	scanf("%c", &selection);
      ...
      play_again = 0;				// so return to the main menu
  }
}
...
int pick_a_number() {
  ...
  printf("Pick a number between1 and 20: ");
  scanf("%d", &pick);
...
// This function is the No Match Dealer game.
...
int find_the_ace() {
  ...
  printf("After choosing a card, one of the two queens will be revealed.\n");
  ...
  while(invalid_choice) {	// Loop until valid choice is made.
    ...
    if(choice_two == "i") { // Increase wager.
      ...
      while(wager_two == -1) // Loop until a valid second wager is made.
